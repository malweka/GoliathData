    ...
    class InsertSqlBuilder : SqlBuilder
    {
            ...
            : base(sqlMapper, entMap)
        {
            foreach (var prop in entMap)
            {
                ...
                if (!prop.IsAutoGenerated && !Columns.ContainsKey(prop.ColumnName))
                {

                    Columns.Add(prop.ColumnName, sqlMapper.CreateParameterName(ParameterNameBuilderHelper.ColumnQueryName(prop.ColumnName, entMap.TableAlias)));
                }
            }
        }
